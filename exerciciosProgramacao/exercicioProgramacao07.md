...
```javascript
//função que calcula orçamento 1
function calculaOrcamento(){

    var gastos = [3600, 950, 620, 38];
    var gastosTotais = gastos[0];
    var salario = 3500;
    var saldo = 0; 
    ...
    do{
        ...
        i++;
    } while(salario >= gastosTotais && i<gastos.length)
    
    saldo = salario - gastosTotais;
...
```javascript
//Função 2 que calcula orçamento
...
if (numero % 2 === 0) {
  ...
  console.log("ímpar e não divisível por 3");
}
```
...
classe FormaGeometrica:
  ...
  Método Construtor(cor):
    Atribui o valor do atributo cor com o valor passado como parâmetro.
...
#### R:
```
função corrida(aceleracao, distancia, velocidade, max, maximoDeTempo):
  tempo = 0
  ...
  enquanto verdadeiro:
    ...
    se distanciaPercorrida >= distancia então
      imprimir("carro completou a corrida em", tempo, "minutos.")
      parar

    se tempo >= maximoDeTempo então
      imprimir("tempo máximo de corrida alcançado. O carro não completou a corrida.")
      parar
...
função MultiplicacaoDeMatrizes(matrizA, matrizB):
    # Checa se o número de colunas de A é igual ao número de linhas de B
    Se tamanho(matrizA[0]) ≠ tamanho(matrizB) então:
        Retornar "As matrizes não podem ser multiplicadas. Elas têm dimensões incompatíveis."
    Senão:
        # Definimos as dimensões da matriz resultante
        linhasA <- tamanho(matrizA)
        colunasA <- tamanho(matrizA[0])
        colunasB <- tamanho(matrizB[0])
        matrizResultado <- novaMatriz(linhasA, colunasB)

        # Loop pra calcular os elementos da matriz resultante
